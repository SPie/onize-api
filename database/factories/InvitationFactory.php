...
$factory->define(InvitationDoctrineModel::class, function (Faker $faker, array $attributes = []) {
    return [
        ...
        InvitationDoctrineModel::PROPERTY_VALID_UNTIL => $attributes[InvitationDoctrineModel::PROPERTY_VALID_UNTIL] ?? (new CarbonImmutable())->addDay(),
        InvitationDoctrineModel::PROPERTY_META_DATA   => !empty($attributes[InvitationDoctrineModel::PROPERTY_META_DATA])
            ? \json_encode($attributes[InvitationDoctrineModel::PROPERTY_META_DATA])
            : \json_encode([]),
